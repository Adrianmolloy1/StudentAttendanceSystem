...
int main(void){
  ...
  while(1){
	...
	if(std1Count + std2Count + std3Count + std4Count + std5Count >=5){
...
int Attendance(){
	int lcd;
	lcdClear(lcd);
	lcdPuts(lcd, "Attendance Full");	

}

void ButtonPress(){
if(digitalRead(Button)==0){
	FullCount = std1Count + std2Count + std3Count + std4Count + std5Count;
		lcdClear(lcd);
		lcdPrintf(lcd, "Attendance is %i", FullCount);
		delay(2000);
		lcdClear(lcd);
		lcdPuts(lcd, "Please Scan card");
		}
